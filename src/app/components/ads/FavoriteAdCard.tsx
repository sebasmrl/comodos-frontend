        ...
        <Link href={`/anuncios/${adData.id}`} passHref legacyBehavior>
          <div className=" rounded overflow-hidden w-full grid grid-cols-1 sm:grid-cols-3 "> {/* max-w-sm */}
            <div className="col-span-3 sm:col-span-1 aspect-video w-full h-full rounded-lg overflow-hidden flex justify-center items-center">
              <Image width={200} height={200} unoptimized={true} className="aspect-video w-full h-auto scale-125 hover:scale-150 duration-300 transition-transform" src={`${CLOUDFRONT_URL}/${adData.images.find(val => val.fieldName = 'main')?.key}?v=${Date.now()}`} alt="Imagen de la propiedad" />
            </div>
            ...
            <div className=" px-2 sm:px-6 py-2 col-span-3 sm:col-span-2 relative ">
              <div className="mb-2">
                ...
                <FavoriteAdButton className="absolute border-none shadow-none right-2 sm:top-2" id={adData.id} />
              </div>
